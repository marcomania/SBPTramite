...
where ciudadano_cod='".$_GET['txtidciudadano']."'";
	...
	$pdf->Image('../../descarga.png', 17, 7, 30 );
		...
		$pdf->Cell(70);
		$pdf->Cell(120,10, 'Report of processed documents',0,0,'C');
		$pdf->Ln(20);
		$pdf->SetFont('Courier','B',12);
		$pdf->Cell(70);
		$pdf->Cell(60,6, 'SENDER',1,0,'P');
		$pdf->SetFont('Courier','',12);
		$pdf->Cell(80,6, $_GET["txtciudadano"],1,1,'P');
		$pdf->Cell(70);
		$pdf->SetFont('Courier','B',12);
		$pdf->Cell(60,6, 'TYPE SENDER',1,0,'P');
		$pdf->SetFont('Courier','',12);
		$pdf->Cell(80,6, $_GET["txttipopersona"],1,1,'P');
		$pdf->Cell(70);
		$pdf->SetFont('Courier','B',12);
		$pdf->Cell(60,6, 'REPORT DATE',1,0,'P');		
		$pdf->SetFont('Courier','',12);
		$hoy = date("Y-m-j");	
		...
		$pdf->SetFont('Courier','B',12);	
	$pdf->Cell(80);
	$pdf->Cell(120,10, 'List of processed documents',0,0,'C');	
	$pdf->Ln(10);	
	$pdf->SetFillColor(232,232,232);
	$pdf->SetFont('Courier','B',10);
	$pdf->Cell(30,6,'CODE',1,0,'C',1);
	$pdf->Cell(212,6,'SUBJECT',1,0,'C',1);
	$pdf->Cell(35,6,utf8_decode('RECEPTION DATE'),1,1,'C',1);
	...
	if ($cont==0) {
			$pdf->Cell(277,6,"No data found",1,1,'C');
	}
