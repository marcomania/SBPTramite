...
if(mysqli_connect_errno()){
	...
	$pdf->SetFont('Courier','B',15);
		...
		$pdf->Cell(40);
		$pdf->Cell(120,10, 'TOTAL PROFIT BY AMOUNT COLLECTED',0,1,'C');
			...
			$pdf->Cell(80);
			$pdf->Cell(120,10, 'REGISTERED DOCUMENTS',0,1,'C');
			$pdf->Ln(20);
			$pdf->SetFont('Courier','B',12);
			$pdf->SetFillColor(255, 195, 0);
			$pdf->Cell(70);
			$pdf->Cell(50,6,'INITIAL DATE: ',1,0,'c',1);
			$pdf->SetFillColor(253, 254, 254);
			if ($_GET["reporte_fecha1"]=="" || $_GET["reporte_fecha2"]=="") {
				$pdf->Cell(100,6,"Undefined",1,1,'c',1);
			}else {
			...
			$pdf->SetFillColor(255, 195, 0);
			$pdf->Cell(70);
			$pdf->Cell(50,6,'FINAL DATE: ',1,0,'c',1);
			$pdf->SetFillColor(253, 254, 254);
			if ($_GET["reporte_fecha1"]=="" || $_GET["reporte_fecha2"]=="") {
				$pdf->Cell(100,6,"Undefined",1,1,'c',1);
			}else {
	...
	$pdf->Cell(45,6,utf8_decode('RECEPTION DATE'),1,0,'C',1);
	$pdf->Cell(192,6,'SUBJECT',1,0,'C',1);
	$pdf->Cell(30,6,utf8_decode('ASSIGNED AREA'),1,1,'C',1);
	$pdf->SetFont('Courier','B',10);
	$cont=0;
		...
		if ($cont==0) {
			$pdf->Cell(-5);
			$pdf->Cell(297,6,"No data found",1,1,'C');
		}
		$pdf->Ln(5);
	$pdf->Output();
?>