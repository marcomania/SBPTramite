	...
	$.ajax({
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				cadena += "<th style = 'text-align: center'>TYPE OF PERSON</th>";
				...
				if(pagina>1){
					...
					paginar += "<li><a href='javascript:void(0)' onclick='listar_ciudadano_vista("+'"'+valor+'","'+(pagina-1)+'"'+")'>Previous</a></li>";
				}
				else{
				...
				if(pagina < numero_paginas){
					paginar += "<li><a href='javascript:void(0)' onclick='listar_ciudadano_vista("+'"'+valor+'","'+(pagina+1)+'"'+")'>Next</a></li>";
...
function revisar_dni_ciudadano(){
	...
	if (nombre.length>0  && apemat.length>0 && apepat.length>0 && fecha.length>0 && direccion.length>0 ) {
		...
		return swal("Registration not completed","","info");
	}
	if (dni.length==0 ) {
	...
	.done(function(resp){
		...
		if (data.length<=0) {
			...
			swal("Sorry, the DNI entered is already being used by another sender","","warning");
		}
	})
}
