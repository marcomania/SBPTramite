	...
	$.ajax({
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				cadena += "<th style = 'text-align: center'>SEX</th>";
				cadena += "<th style = 'text-align: center'>DATE OF BIRTH</th>";
				cadena += "<th style = 'text-align: center'>EMPLOYMENT</th>";
				cadena += "<th style = 'text-align: center'>USER</th>";
				...
				if(pagina>1){
					...
					paginar += "<li><a href='javascript:void(0)' onclick='listar_personal_vista("+'"'+valor+'","'+(pagina-1)+'"'+")'>Previous</a></li>";
				}
				else{
				...
				if(pagina < numero_paginas){
					paginar += "<li><a href='javascript:void(0)' onclick='listar_personal_vista("+'"'+valor+'","'+(pagina+1)+'"'+")'>Next</a></li>";
...
function Editar_personal(){
	...
	if(nombre.length>0 && apepat.length>0 && apemat.length>0 && direccion.length>0 && nrodocume.length>0 && email.length>0 ){
		...
		return swal("Missing Fill Data", "", "info");	
	}	
	$.ajax({
	...
	.done(function(resp){
		if (resp > 0) {
			...
			swal("! Registration not completed¡", "", "error");	
		}
	})
...
function revisar_dni_personal(){
	...
	if (dni.length==0 ) {
		return swal("Missing Fill Your Identity Document","","info");
	}
	if (usuario.length==0) {
		return swal("Missing Fill Your Username","","info");
	}
	if (clave.length==0) {
		return swal("Missing Fill Your Key","","info");
	}
	if (tipo=='otro') {
		return swal("Missing Select User Type","","error");
	}
	$.ajax({
	...
	.done(function(resp){
		...
		if (data.length<=0) {
			...
			swal("Sorry, the DNI entered is already being used by other personnel","","warning");
		}
	})
}
...
function Listar_tipousuario_combo() {
	...
	.done(function(resp) {
		...
		if (data.length > 0) {
			var cadena = "";
				cadena += "<option value='otro'>"+"SELECT USER TYPE"+"</option>";
			...
			var cadena = "<option value='NO'>no available user type found</option>";
			$("#cmbTipousuario").html(cadena);
		}
	})
}